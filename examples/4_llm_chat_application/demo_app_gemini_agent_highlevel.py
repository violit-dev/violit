...
Do not claim that image analysis is impossible unless the image bytes are truly unavailable.
If the user's question is about the attached image, answer from the image itself first, then incorporate any tool evidence when relevant.
""".strip()
    return f"""
    ...
    def stream():
        ...
        try:
            ...
            for step_index in range(1, MAX_AGENT_STEPS + 1):
                plan = _call_gemini_json(key, _build_planner_prompt(prompt_text, tool_history, step_index, source_prompt=prompt), source_prompt=prompt)
                ...
                action_input: dict[str, Any] = dict(plan.get("action_input") or {}) if isinstance(plan.get("action_input"), dict) else {}
            ...
            final_prompt = _build_final_answer_prompt(prompt_text, tool_history, final_summary, source_prompt=prompt)
            if answer_mode == "streaming":
                saw_text = False
                for text in _stream_gemini_answer(key, final_prompt, source_prompt=prompt):
                    saw_text = True
                ...
                yield {"type": "text", "text": _generate_gemini_answer(key, final_prompt, source_prompt=prompt), "stream": False}
